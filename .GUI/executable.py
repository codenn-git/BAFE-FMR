...
class RoadDelineationPipeline:
    ...
    def __init__(self, raster_path, vector_path, output_base_dir, BSG=True):
        """
    ...
    def preprocess_image(self):
        ...
        print(f"[{self.vector_basename}] Preprocessing [{self.image_type}] image...")
        if not self.bsg: #for PNEO
    ...
    def extract_centerline(self, preprocessor, spacing=5):
        ...
        print(f"[{self.vector_basename}] Skeleton method: {progress_skeleton:.1f}% ({status_skeleton})")
        
        # Check if skeleton method is erroneous
        if status_skeleton == "Erroneous Processing":
            print(f"[{self.vector_basename}] Skeleton method erroneous, falling back to transect-based method...")
            ...
            print(f"[{self.vector_basename}] Transect method: {progress_percent:.1f}% ({status})")
            ...
            print(f"[{self.vector_basename}] Skeleton method successful!")
        ...
        self._save_figure(
            ...
            "04_centerline.png",
            f"Extracted Centerline ({self.results['method']})",
    ...
    def generate_polygon(self, road_mean_width):
        ...
        if self.centerline is None:
            raise ValueError("Centerline must be extracted first. Run extract_centerline() before generate_polygon().")
        ...
        self._save_figure(
            ...
            self.clipped_transform,
            "05_road_polygon.png",
            f"Road Polygon (from {self.results['method']} centerline)",
...
def main():
    ...
    # BSG Example
    raster_path = r"C:\Users\user-307E4B3400\OneDrive - Philippine Space Agency\SDMAD_SHARED\PROJECTS\SAKA\FMR\PNEO FMR\.PANSHARPENED\PNEO3_05323315308984\FMR 38\FMR38-PANSHARPENED.tif"
    vector_path = r"C:\Users\user-307E4B3400\OneDrive - Philippine Space Agency\SDMAD_SHARED\PROJECTS\SAKA\FMR\PNEO FMR\.PANSHARPENED\PNEO3_05323315308984\FMR 38\FMR38-PNEO-CL.shp"
