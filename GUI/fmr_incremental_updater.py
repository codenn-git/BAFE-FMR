...
class IncrementalUpdater:
    ...
    def __init__(self, shapefile_path, bsg_folder):
        ...
        self.db_path = os.path.join(os.path.dirname(shapefile_path), "fmr_database_migo.csv")
    ...
    # 10/28: writes geometry columns for new raster matches
    def process_new_rasters(self, new_rasters):
        ...
        if os.path.exists(self.db_path):
            ...
            existing_keys = set(zip(existing_df.get("FMR", []),
                                    existing_df.get("BSG", []),
            ...
            try:
                with rasterio.open(tif_path) as src:
                    ...
                    for idx, row in fmr_gdf.iterrows():
                        ...
                        # Check for nodata coverage (same logic as full scan)
                        geom_proj = gpd.GeoSeries([fmr_geom], crs=fmr_gdf.crs).to_crs(src.crs)
                        fmr_line = geom_proj.iloc[0]

                        num_samples = max(20, int(fmr_line.length // 10))
                        distances = np.linspace(0, fmr_line.length, num_samples)
                        xs = []
                        for d in distances:
                            p = fmr_line.interpolate(d)
                            xs.append((p.x, p.y))
                        values = list(src.sample(xs))
                        ...
                        # 10/28: geometry columns
                        minxg, minyg, maxxg, maxyg = fmr_geom.bounds
                        c = fmr_geom.centroid
                        ...
                        new_entries.append({
                            ...
                            "Processing Type": "",
                            "Image Path": tif_path,
                            # 10/28: geometry to CSV
                            "GEOM_WKT": fmr_geom.wkt,
                            "EXTENT_MINX": minxg, "EXTENT_MINY": minyg,
                            "EXTENT_MAXX": maxxg, "EXTENT_MAXY": maxyg,
        ...
        if new_entries:
            ...
            if not existing_df.empty:
                # 10/28: ensure legacy CSV gets new geometry columns
                for col in ["GEOM_WKT","EXTENT_MINX","EXTENT_MINY","EXTENT_MAXX","EXTENT_MAXY","CENTROID_X","CENTROID_Y","Processing Type"]:
